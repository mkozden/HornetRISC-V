...
jmp_buf ovfl_buf;
#ifdef __STDC__
//#include <stdlib.h> //Only used for exit function, which we can define ourselves
#include <math.h>
#else
...
FLOAT Eight = 8.0f;
FLOAT Nine = 9.0f;
FLOAT TwentySeven = 27.0f;
FLOAT ThirtyTwo = 32.0f;
FLOAT TwoForty = 240.0f;
FLOAT MinusOne = -1.0f;
...
//   {
//     //write_string("Precision cannot be characterized by an Integer number\n");
//     //write_string("of significant digits but, by itself, this is a minor flaw.\n");
//   }

//   if (Radix == One) 
//   {
//     //write_string("logarithmic encoding has precision characterized solely by U1.\n");
//   }
//   else
//   {
//     ////write_string("The number of significant digits of the Radix is %f .\n", Precision);
//   }
...
/*=============================================*/
  ...
  if ((X1 != Y1) || (X1 != Z1)) {
    ...
    //write_string("respectively  %.7e,  %.7e,  %.7e,\n", X1, Y1, Z1);
    //write_string("are symptoms of inconsistencies introduced\n");
    //write_string("by extra-precise evaluation of arithmetic subexpressions.\n");
    ...
    if ((Z1 != U1) || (Z2 != U2)) {
      if ((Z1 >= U1) || (Z2 >= U2)) {
        ...
        if (Z1 != Z2 || Z1 > Zero) {
          ...
          if (Y > X) X = Y;
          //Q = - LOG(X);
          //write_string("Some subexpressions appear to be calculated extra\n");
          ////write_string("precisely with about %g extra B-digits, i.e.\n",
          //  (Q / LOG(Radix)));
          ////write_string("roughly %g extra significant decimals.\n",
          //  Q / LOG(10.f));
          }
        //write_string("That feature is not tested further by this program.\n");
        }
      }
    }
  Pause();
...
or  1/3  and  3/9  and  9/27 may disagree");
  ...
  if (F9 != One && F9 - One >= Zero) {
    ...
    //write_string("  subtraction yields  (1-U1) - 1 = 0 , thereby vitiating\n");
    //write_string("  such precautions against division by zero as\n");
    //write_string("  ...  if (X == 1.0) {.....} else {.../(X-1.0)...}\n");
    }
...
/*=============================================*/
  ...
  ////write_string("Testing on %d random pairs.\n", NoTrials);
  Random9 = SQRT(3.0f);
  Random1 = Third;
  I = 1;
  do  {
...
/*=============================================*/
  ...
  SqXMinX (Serious);
  if (J != Zero) Pause();
  ////write_string("Testing if sqrt(X * X) == X for %d Integers X.\n", NoTrials);
  J = Zero;
  X = Two;
  Y = Radix;
  if ((Radix != One)) do  {
...
//   RSqrt = Other; /* ~dgh */
//   if (Radix != One) {
//     //write_string("Testing whether sqrt is rounded or chopped.\n");
...
//       ////write_string("Radix^Precision = %.7e\n", W);
//       //write_string(" fails test whether sqrt rounds or chops.\n");
//       SqRWrng = True;
//       }
//     }
//   if (! Anomaly) {
...
//       RSqrt = Rounded;
//       //write_string("Square root appears to be correctly rounded.\n");
//       }
//     else  {
...
//       else {
//         RSqrt = Chopped;
//         //write_string("Square root appears to be chopped.\n");
//         }
//       }
//     }
//   if (SqRWrng) {
//     //write_string("Square root is neither chopped nor correctly rounded.\n");
//     ////write_string("Observed errors run from %.7e ", MinSqEr - Half);
//     ////write_string("to %.7e ulps.\n", Half + MaxSqEr);
...
/*=============================================*/
//   Pause();
//   write_string("Testing powers Z^i for small Integers Z and i.\n");
//   N = 0;
// /* ... test powers of zero. */
//   I = 0;
//   Z = -Zero;
...
//     } while ( ! (Break));
  ...
  //   } while ( Z < Eight * Three );
  // if (N > 0) {
  //   write_string("Errors like this may invalidate financial calculations\n");
  //   write_string("\tinvolving interest rates.\n");
  //   }
  // PrintIfNPositive();
  // N += N1;
  // if (N == 0) //write_string("... no discrepancies found.\n");
  // if (N > 0) Pause();
  // else //write_string("\n");
  /*=============================================*/
  Milestone = 110;
...
/* ... PseudoZero < Underflow or PseudoZero < PseudoZero + PseudoZero
    ...
    if (PseudoZero <= Zero) {
      ...
      ////write_string("PseudoZero that prints out as: %g .\n", PseudoZero);
      X = - PseudoZero;
      if (X <= Zero) {
...
/*=============================================*/
  ...
  switch (I)  {
    ...
    if ((CInvrse * Q) != ((CInvrse * Y) * S)) {
      ...
      ////write_string("approach a threshold = %.17e\n", UfThold);;
      ////write_string(" coming down from %.17e\n", C);
    ...
    ////write_string("print out as Q = %.17e, Y = %.17e .\n", Q, Y2);
    ////write_string ("|Q - Y| = %.17e .\n" , FABS(Q - Y2));
    UfThold = Q;
    break;
  ...
  if (UfNGrad) {
    ...
    if (! ((X == Z) || (X - Z != Zero))) {
      ...
      //write_string("    if (X == Z)  ...  else");
      //write_string("  ... (f(X) - f(Z)) / (X - Z) ...\n");
      //write_string("encounter Division by Zero although actually\n");
      sigsave = sigfpe;
      if (setjmp(ovfl_buf)) write_string("X / Z fails!\n");
      else ////write_string("X / Z = 1 + %g .\n", (X / Z - Half) - Half);
      sigsave = 0;
      }
    }
  ////write_string("The Underflow threshold is %.17e, %s\n", UfThold,
  //     " below which");
  //write_string("calculation may suffer larger Relative error than ");
  //write_string("merely roundoff.\n");
  Y2 = U1 * U1;
  Y = Y2 * Y2;
  Y2 = Y * U1;
  if (Y2 <= UfThold) {
    ...
    ////write_string("Range is too narrow; U1^%d Underflows.\n", I);
    }
  /*=============================================*/
  Milestone = 130;
  /*=============================================*/
  ...
  // Y2 = Y + Y;
  // //write_string("Since underflow occurs below the threshold\n");
  // ////write_string("UfThold = (%.17e) ^ (%.17e)\nonly underflow ", HInvrse, Y);
  // ////write_string("should afflict the expression\n\t(%.17e) ^ (%.17e);\n",
  // //  HInvrse, Y2);
  // //write_string("actually calculating yields:");
  // if (setjmp(ovfl_buf)) {
  //   sigsave = 0;
  ...
  //   ////write_string("   threshold = %.17e .\n", UfThold);
  //   }
  //   else if (! (V9 > UfThold * (One + E9)))
  //     write_string("This computed value is O.K.\n");
  //   else {
  ...
  // X = F9;
  // Y = X - U1;
  // ////write_string("Testing X^((X + 1) / (X - 1)) vs. exp(2) = %.17e as X -> 1.\n",
  // //  Exp2);
  // for(I = 1;;) {
  //   Z = X - BInvrse;
  ...
  //     ////write_string(" %.17e for\n", POW(X,Z));
  //     ////write_string("\t(1 + (%.17e) ^ (%.17e);\n", V9, Z);
  //     ////write_string("\tdiffers from correct value by %.17e .\n", Q);
  //     //write_string("\tThis much error may spoil financial\n");
  //     //write_string("\tcalculations involving tiny interest rates.\n");
  //     break;
  //     }
  //   else {
  //     Z = (Y - X) * Two + Y;
  ...
  //       if (X > One) {
  //         if (N == 0)
  //            //write_string("Accuracy seems adequate.\n");
  //         break;
  //         }
  //       else {
  //         X = One + U2;
  ...
  Milestone = 150;
  /*=============================================*/
  // write_string("Testing powers Z^Q at four nearly extreme values.\n");
  // N = 0;
  // Z = A1;
...
overflow:
  ...
  ////write_string("Trying it on Y = %.17e .\n", Y);
  V9 = - Y;
  V0 = V9;
  ...
  {
    BadCond(Serious, "");
    ////write_string("overflow past %.17e\n\tshrinks to %.17e .\n", Y, Z);
    }
  if (I)
  {
  ...
  ////write_string("Overflow threshold is V  = %.17e .\n", V);
  if (I) write_string("Overflow saturates at V0 = %.17e .\n");
  else write_string("There is no saturation value because \
the system traps on overflow.\n");
  V9 = V * One;
  ////write_string("No Overflow should be signaled for V * 1 = %.17e\n", V9);
  V9 = V / One;
  ////write_string("                           nor for V / 1 = %.17e .\n", V9);
  //write_string("Any overflow signal separating this * from the one\n");
  //write_string("above is a DEFECT.\n");
  /*=============================================*/
  Milestone = 170;
  /*=============================================*/
  ...
  for(Indx = 1; Indx <= 2; ++Indx) {
    ...
    if (((V9 < (One - Two * Radix * E9) * Z) || (V9 > Z)))
    {
      ...
      ////write_string("Comparison alleges that Z = %17e\n", Z);
      ////write_string(" is too far from sqrt(Z) ^ 2 (%.17e) .\n", Y);
      }
    }
  /*=============================================*/
...
#ifndef NOPAUSE
  //write_string("This can interupt your program.  You can ");
  //write_string("skip this part if you wish.\n");
  //write_string("Do you wish to compute 1 / 0? ");
  fflush(stdout);
  ch[0] = 'Y';
  if ((ch[0] == 'Y') || (ch[0] == 'y')) {
#endif
...
/*=============================================*/
  ...
  //     //write_string("The number of  %-29s %d.\n",
  //       msg[i], ErrCnt[i]);
  //   }
  //write_string("\n");
  ...
  {
    //write_string("No failures, defects nor flaws have been discovered.\n");
...
//      fpecount);
  ...
  //write_string ( "\n" );
  //write_string ( "PARANOIA:\n" );
  //write_string ( "  Normal end of execution.\n" );
...
*/
{
  if (Y != X)
  {
    if (N <= 0)
    {
      ...
      if (Z == Zero && Q <= Zero)
        write_string ( "WARNING:  computing\n" );
      else 
        BadCond(Defect, "computing\n");

//      //write_string ( "\t(%.17e) ^ (%.17e)\n", Z, Q);
//      //write_string ( "\tyielded %.17e;\n", Y);
//      //write_string ( "\twhich compared unequal to correct %.17e ;\n",
//        X);
//      //write_string ( "\t\tthey differ by %.17e .\n", Y - X);
    }
/*
...
*/
{
  if ( N > 0 ) 
  {
    //write_string("Similar discrepancies have occurred N>0 times.\n");
  }
  return;
}
/**********************************************************************/
...
very_serious:
        ...
        //write_string("This is a VERY SERIOUS DEFECT!\n");
      }
      else
      {
        N = 1;
