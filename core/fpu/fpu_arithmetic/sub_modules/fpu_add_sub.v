...
reg        f12_sign_O;
reg        f12_sign_less;
reg        f12_second_operand_zero;
reg        f12_mux_fastres_sel;
reg [31:0] f12_fast_res;
reg        f12_overflow_fast;
reg        f12_invalid_fast;

always @ (posedge clk or negedge reset) begin
    if(!reset) begin
        f12_out_sig_abs         <= 28'b0;
        f12_exp_O               <= 8'b0;
        f12_sign_O              <= 1'b0;
        f12_sign_less           <= 1'b0;
        f12_second_operand_zero <= 1'b0;
        f12_mux_fastres_sel     <= 1'b0;
        f12_fast_res            <= 32'b0;
...
assign LGRS = f12_out_sig_abs[3:0];
...
add_sub_normalizer add_sub_normalizer(.inSig(f12_out_sig_abs), .inExp(f12_exp_O), .LGRS(LGRS), .outExp(pro_normal_exp), .of(ofFromProNorm), .uf(ufFromProNorm), .out_sig(pro_normal_sig));
fpu_add_sub_rounder fpu_add_sub_rounder( .LRS(LRS),
                                         .rounding_mode(rounding_mode),
                                         .second_operand_zero(f12_second_operand_zero),
                                         .sign_less(f12_sign_less),
                                         .sign_O(f12_sign_O),
                                         .round_out(round_out));
...
assign inexact   = f12_mux_fastres_sel ? f12_overflow_fast : |LRS[1:0] | of | uf;
assign invalid   = f12_invalid_fast;
assign overflow  = f12_mux_fastres_sel ? f12_overflow_fast : of;
assign underflow = uf;
...
always @(*) begin
    if(overflow) begin //Overflow edge cases
        if(f12_sign_O) begin //For negative numbers
            ...
            else OUT_round = round_out[1] ? {f12_sign_O, final_exp, final_sig} - round_out[0] : {f12_sign_O, final_exp, final_sig} + round_out[0];
        end
...
assign OUT       = f12_mux_fastres_sel ? f12_fast_res : OUT_round;
