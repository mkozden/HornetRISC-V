...
assign fcsr = {24'b0, frm[2:0], fflags_accum};
assign fpu_dyn_rm = frm[2:0];

// The register updates on the same posedge the tracer samples, so the tracer needs this pre-update `| delta` combination to print the correct post-op value.
assign fflags_accum_o = fflags_accum | wb_fflags_i[4:0];
...
begin
    ...
    begin
        ...
        begin
            ...
            begin
                // When writing to frm, the input can be directly passed here
                frm <= csr_reg_i;
            end

            else if(csr_w_addr_i[11:0] == 12'h003) //0x003 - fcsr
            begin
                // When writing to fcsr, the bits 7:5 are written to frm
                frm <= {29'b0, csr_reg_i[7:5]};
            end
        ...
        // The FFLAGS accumulation logic is handled separately from the others,
        // with a non-conditional path to keep accumulating
        if(!csr_wen_i && csr_w_addr_i[11:0] == 12'h001) //0x001 - fflags
          fflags_accum <= csr_reg_i[4:0] | wb_fflags_i[4:0]; // We OR the incoming flags with the CSR input, architecturally only one can be active at a time
        else if(!csr_wen_i && csr_w_addr_i[11:0] == 12'h003) //0x003 - fcsr
          fflags_accum <= csr_reg_i[4:0] | wb_fflags_i[4:0]; // Identical to above
        else
          fflags_accum <= fflags_accum | wb_fflags_i[4:0]; // Accumulate the register value
    end
end
